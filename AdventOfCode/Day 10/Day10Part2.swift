...
private struct Coordinate {
    ...
    var cgPoint: CGPoint {
        CGPoint(x: location.0, y: location.1)
    }
...
private enum Pipe: String {
    ...
    var character: Character {
        switch self {
            ...
            case .edge: return self.rawValue.first!
        }
    }
}

public func day10Part2(_ input: String) -> Int {
    var input = input
    ...
    path.move(to: startingPoint.cgPoint)

    while true {
        // Mark as visited
        if loopStep.pipe != startingPoint.pipe {
            input[loopStep.location.0][loopStep.location.1] = Pipe.edge.character
        }
        ...
        guard loopStep.pipe != .pS else {
            path.closeSubpath()
            break
        }

        path.addLine(to: loopStep.cgPoint)
    }

    input[startingPoint.location.0][startingPoint.location.1] = Pipe.edge.character

    return floodCount(input, path)
}

private func floodCount(_ input: [[Character]], _ path: CGPath) -> Int {
    var count = 0

    for i in (0..<input.count) {
        for j in (0..<input[i].count) where path.contains(CGPoint(x: i, y: j)) && input[i][j] != Pipe.edge.rawValue.first! {
            count += 1
        }
    }
