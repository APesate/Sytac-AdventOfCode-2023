...
public func day17Part2(_ input: String) -> Int {
    ...
    return dijkstra(grid: input, start: Point(x: 0, y: 0), end: Point(x: lines.count - 1, y: lines.first!.count - 1), maxRun: 10) ?? -1
}
...
private func neighbours(_ state: State, grid: [Point: Int]) -> [State] {
    ...
    if 0 < consecutiveMovesRow && consecutiveMovesRow < 4 {
        result.append(State(point: Point(x: i + 1, y: j), run: (consecutiveMovesRow + 1, 0)))
    } else if -4 < consecutiveMovesRow && consecutiveMovesRow < 0 {
        result.append(State(point: Point(x: i - 1, y: j), run: (consecutiveMovesRow - 1, 0)))
    } else if 0 < consecutiveMovesCol && consecutiveMovesCol < 4 {
        result.append(State(point: Point(x: i, y: j + 1), run: (0, consecutiveMovesCol + 1)))
    } else if -4 < consecutiveMovesCol && consecutiveMovesCol < 0 {
        result.append(State(point: Point(x: i, y: j - 1), run: (0, consecutiveMovesCol - 1)))
    } else {
        result.append(contentsOf: neighboursPart1(state, grid: grid))
    }

    return result.filter { grid[$0.point] != nil }
}

private func neighboursPart1(_ state: State, grid: [Point: Int]) -> [State] {
    let (i, j) = (state.point.x, state.point.y)
    let (consecutiveMovesRow, consecutiveMovesCol) = state.run
    var result: [State] = []
